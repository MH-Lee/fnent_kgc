...
class DualAttnE(Model):
    ...
    def init_emb(self, args):
        ...
        self.ffn_output = nn.Sequential(nn.Linear(2*self.emb_dim, self.args.dim_feedforward),
                                        nn.GELU(),
    ...
    def score_func(self, head_emb, relation_emb, choose_emb = None):
        ...
        for layer in range(self.args.nblocks):
            ...
            if layer <= (self.args.nblocks - 1):
                ...
                head_emb = torch.stack([real_attn_ent, imag_attn_ent], dim=0) # [2, B, 1, emb_dim]
                head_emb = head_emb.norm(dim = 0) # [B, 1, emb_dim]
                relation_emb = torch.stack([real_attn_rel, imag_attn_rel], dim=0) # [2, B, 1, emb_dim]
                relation_emb = relation_emb.norm(dim = 0) # [B, 1, emb_dim]
        ...
        # merged_out = torch.stack([real_attn_ent, imag_attn_ent], dim=0) # [2, B, 1, emb_dim]
        # merged_out = merged_out.norm(dim=0)
        merged_out = torch.cat([re_score, im_score], dim=-1)
        # import pdb;pdb.set_trace()
